...
interface CoachApplication {
  ...
  email: string;
  phone?: string;
  applicationType: 'COACH' | 'INSTRUCTOR' | 'TI' | 'AFFI';
  disciplines: string[];
  languages?: string[];
  totalJumps?: number;
  experience?: string;
...
const FALLBACK_COACHES: CoachApplication[] = [];
...
export default function AdminCoachesOnboardingPage() {
  const { user } = useAuth();
  ...
  const dzLabel = (app: CoachApplication) => {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet<{ success: boolean; data: CoachApplication[] }>('/onboarding/coaches');
      if (res.success && res.data && res.data.length > 0) {
        setApplications(
          (res.data as unknown as Array<{ id: number } & Record<string, unknown>>).map((row) => ({
            ...row,
            id: String(row.id),
          })) as CoachApplication[]
        );
      ...
      setError('Failed to load coach applications. Please try again.');
      setApplications(FALLBACK_COACHES);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleReject = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      await apiPost(`/onboarding/coaches/${id}/reject`, { notes });
      setApplications((prev) =>
  ...
  const handleAssignDropzone = async (id: string) => {
    ...
    setError(null);
    try {
      await apiPost(`/onboarding/coaches/${id}/assign-dropzone`, { dropzoneId: dzId });
  ...
  const handleRequestDocs = async (id: string) => {
    setActionLoading(id);
    try {
      await apiPost(`/onboarding/coaches/${id}/request-docs`, { notes: 'Additional documents required.' });
      setApplications((prev) =>
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review and manage coach onboarding applications</p>
        </div>
        <button
            ...
            <div className="p-6 space-y-6">
              ...
              {selectedApp.ratingDetails && (
                <section>
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300">{selectedApp.ratingDetails}</p>
                </section>
              )}
