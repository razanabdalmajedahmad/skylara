    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <button
          onClick={async () => {
            try {
              await navigator.clipboard.writeText(`${window.location.origin}/onboarding/coaches/register${registerQuery}`);
              setLinkToast('Coach registration link copied!');
            } catch {
        ...
        >
          <LinkIcon className="w-3.5 h-3.5" /> Share Coach Link
        </button>
      </div>
      <div className="space-y-3">
        ...
        {filtered.length === 0 && <EmptyState message="No coach applications found" />}
      </div>
      {(toast || linkToast) && (
...
function DocumentsPanel({ docs, showToast }: { docs: DocItem[]; showToast: (msg: string) => void }) {
  ...
  return (
    <SectionCard title="Coach Documents" action={<button onClick={() => showToast('Upload request sent to coach')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700"><Upload className="w-3.5 h-3.5" /> Request Upload</button>}>
      <div className="space-y-3">
        {docs.map((d) => (
...
function WaiversPanel({ waivers }: { waivers: WaiverItem[] }) {
  return (
    <SectionCard title="Coach Waivers & Agreements">
      <div className="space-y-3">
        {waivers.map((w) => (
...
function LicensesPanel({ licenses, showToast }: { licenses: LicenseItem[]; showToast: (msg: string) => void }) {
  return (
    <SectionCard title="Coach Licenses & Ratings" action={<button onClick={() => showToast('Rating upload initiated')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700"><Upload className="w-3.5 h-3.5" /> Upload Rating</button>}>
      <div className="space-y-3">
        {licenses.map((l) => (
...
function ReviewRulesPanel({ rules }: { rules: RuleItem[] }) {
  return (
    <SectionCard title="Coach Review Rules" action={<Link href="/dashboard/onboarding/templates" className="px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700">Templates &amp; rules</Link>}>
      <div className="space-y-3">
        {rules.map((r) => (
...
function NotificationsPanel({ notifs }: { notifs: NotifItem[] }) {
  return (
    <SectionCard title="Coach Notifications">
      <div className="space-y-3">
        {notifs.map((n) => (
...
function HistoryPanel({ history }: { history: HistoryItem[] }) {
  return (
    <SectionCard title="Coach Activity History">
...
export default function CoachesPage() {
  ...
  const { ctx, loading: ctxLoading, error: ctxError } = useSegmentContext('coaches');

  const coachGoalsMerged = useMemo((): CoachGoalFlags => ({
    ...EMPTY_COACH_GOALS,
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await apiGet<{ success: boolean; data: any[] }>('/onboarding/coaches');
  ...
  const handleAction = async (id: string, action: 'approve' | 'request_info' | 'reject') => {
    try {
      if (action === 'approve') {
        await apiPost(`/onboarding/coaches/${id}/approve`, {});
      } else if (action === 'request_info') {
        await apiPost(`/onboarding/coaches/${id}/request-docs`, { notes: 'Additional information requested' });
      } else {
        await apiPatch(`/onboarding/coaches/${id}`, { status: 'ARCHIVED', reviewNotes: 'Application archived' });
      }
      setToast('Coach application updated');
  ...
  if (loading) return <PageLoading label="Loading coach applications..." />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Coach Onboarding</h2>
      </div>
      ...
      {activeSubTab === 'goals' && (ctxLoading ? <PageLoading label="Loading goals..." /> : <GoalsPanel goals={coachGoalsMerged} />)}
