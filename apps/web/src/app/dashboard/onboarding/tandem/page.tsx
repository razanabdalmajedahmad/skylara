...
interface InterestFlags { wantsCoachPath: boolean; wantsInstructorPath: boolean; wantsTunnelCoaching: boolean; wantsCompetition: boolean; wantsEvents: boolean; wantsCamps: boolean; wantsAff: boolean; wantsFreefly: boolean; wantsWingsuit: boolean; wantsCanopyCoaching: boolean }
...
function ApplicationsPanel({ data, search, setSearch, filter, setFilter, filters, handleAction }: {
  data: CategoryApplication[]; search: string; setSearch: (v: string) => void;
  filter: string; setFilter: (v: string) => void; filters: FilterChip[];
  handleAction: (id: string, action: 'approve' | 'request_info' | 'reject') => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1"><SearchInput value={search} onChange={setSearch} placeholder="Search customers..." /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">No tandem customers found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
...
function DocumentsPanel({ docs, showToast }: { docs: DocItem[]; showToast: (msg: string) => void }) {
  ...
  return (
    <SectionCard title="Tandem Customer Documents" action={<button onClick={() => showToast('Upload request sent to customer')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700"><Upload className="w-3.5 h-3.5" /> Request Upload</button>}>
...
function MedicalPanel({ medical }: { medical: { declared: boolean; riskFlags: string[]; reviewedBy: string | null } | null }) {
  ...
  return (
    <SectionCard title="Medical Declarations">
      <div className="space-y-4">
        {!status.declared && !status.reviewedBy ? (
          ...
          <>
            <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
              ...
              <div>
                <p className="text-sm font-semibold text-green-800 dark:text-green-300">Medical Declaration Complete</p>
...
function GoalsPanel({ interests }: { interests: InterestFlags }) {
  const flags = [
    ...
    { key: 'wantsTunnelCoaching', label: 'Tunnel Coaching', value: interests.wantsTunnelCoaching },
    { key: 'wantsCompetition', label: 'Competition', value: interests.wantsCompetition },
    { key: 'wantsEvents', label: 'Events & Boogies', value: interests.wantsEvents },
    { key: 'wantsCamps', label: 'Coaching Camps', value: interests.wantsCamps },
    { key: 'wantsAff', label: 'AFF Training', value: interests.wantsAff },
    ...
    { key: 'wantsCanopyCoaching', label: 'Canopy Coaching', value: interests.wantsCanopyCoaching },
  ];
...
export default function TandemPage() {
  ...
  const { ctx, loading: ctxLoading, error: ctxError } = useSegmentContext('tandem');

  const interestsMerged = useMemo((): InterestFlags => ({
    ...EMPTY_INTERESTS,
    ...(ctx?.athleteInterests || {}),
  }), [ctx]);
  ...
  const fetchData = useCallback(async () => {
    try { setData(await apiGet<CategoryApplication[]>('/onboarding/tandem')); } catch { setData([]); }
    setLoading(false);
  }, []);
  ...
  if (loading) return <PageLoading label="Loading tandem customers..." />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Tandem Customer Onboarding</h2>
        <button
          onClick={async () => {
            try {
              const link = `${window.location.origin}/bookings/tandem`;
              await navigator.clipboard.writeText(link);
              setToast('Tandem booking link copied to clipboard!');
            } catch {
        ...
        >
          <ExternalLink className="w-3.5 h-3.5" /> Share Booking Link
        </button>
      </div>

      <SubTabs tabs={tabsWithCounts} active={activeSubTab} onSwitch={setActiveSubTab} />

      {activeSubTab === 'applications' && <ApplicationsPanel data={data} search={search} setSearch={setSearch} filter={filter} setFilter={setFilter} filters={filters} handleAction={handleAction} />}
