  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1"><SearchInput value={search} onChange={setSearch} placeholder="Search managers..." /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">No manager applications found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
...
function DocumentsPanel({ docs, showToast }: { docs: DocItem[]; showToast: (msg: string) => void }) {
  ...
  return (
    <SectionCard title="Manager Documents" action={<button onClick={() => showToast('Upload request sent to manager')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700"><Upload className="w-3.5 h-3.5" /> Request Upload</button>}>
...
export default function ManagersPage() {
  ...
  const { ctx, loading: ctxLoading, error: ctxError } = useSegmentContext('managers');
  ...
  const fetchData = useCallback(async () => {
    try { setData(await apiGet<CategoryApplication[]>('/onboarding/managers')); } catch { setData([]); }
    setLoading(false);
  }, []);
  ...
  if (loading) return <PageLoading label="Loading managers..." />;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">DZ Manager Onboarding</h2>
        <button
          onClick={async () => {
            try {
              const link = `${window.location.origin}/onboarding/managers/register`;
              await navigator.clipboard.writeText(link);
              setToast('Manager registration link copied to clipboard!');
            } catch {
