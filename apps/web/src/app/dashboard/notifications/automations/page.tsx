...
const emptyForm: AutoForm = { name: '', category: 'NOTIFICATION', triggerEvent: 'USER_REGISTERED' };

export default function NotifAutomationsPage() {
        ...
        <div className="bg-white dark:bg-slate-800 dark:bg-gray-800 rounded-xl border-2 border-blue-500 p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Welcome new athletes" className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 dark:border-gray-600 rounded-lg bg-white dark:bg-slate-800 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    ...
                    {aut.lastRunAt && <> &middot; Last: {new Date(aut.lastRunAt).toLocaleDateString()}</>}
                    &middot; {aut.runCount} runs
                  </p>
                </div>
                <div className="flex items-center gap-2">
